...
#  output$Go <- eventReactive(input$Go,  
    {
      ...
      #removeUI(selector="CANDIDATES")
      #removeUI(selector="SMILES")
      Bypass = TRUE 
      ...
      #1. Query iLINCS for user input gene
        if (input$Signature == "Define Target Gene"){
          ...
          #removeUI(selector="output$CANDIDATES")
          #emoveUI(selector="output$SMILES")
          ...
          # #################
           ...
           colnames(lincs_output) <- c("Candidate LSM ID", "Candidate Name", "Cell Line", "Concordance", "Significance")
           lincs_output <- lincs_output[,-5]
           lincs_output <- lincs_output[,c(2,1,3,4)]
           shinyjs::showElement("SMILES")
           output$SMILES <<- DT::renderDataTable({
             lincs_output$`Candidate LSM ID`<- makeLink5(lincs_output$`Candidate LSM ID`)
             lincs_output
          ...
          shinyjs::showElement("SMILES")
          output$SMILES <<- DT::renderDataTable({
            connected_sigs$`Candidate LSM ID`<- makeLink5(connected_sigs$`Candidate LSM ID`)
            connected_sigs
            return(connected_sigs)}, caption="LINCS Candidates Ranked", escape=FALSE, rownames=FALSE)
          #UploadConDiscon(rjson)
          load("./lincs_fps.RData")
          # 
          lsm_rows <<- which(rownames(lincs_fps_2) %in% connected_sigs$`Candidate LSM ID`)
          display <<- connected_sigs[,c(2,1)]
          max_cons_4 <- connected_sigs
          ...
          #    downloadButton("LINCSDownload_upload", label = "LINCS Candidates Ranked")})
...
#        output$distPlot <<- renderPlot(heatmap.2(1-simMA, Rowv=as.dendrogram(hc), Colv=as.dendrogram(hc), col=colorpanel(40, "white","yellow","red"), density.info="none", trace="none", labCol=cid(fpset), labRow=cid(fpset), cexRow=0.5, cexCol=0.5))
  ...
  observeEvent(input$Cluster,{
    withProgress(message = "Clustering", value = 0, {
      shinyjs::hideElement("cluster_check")
      ...
      if (index_analogs == 1){
        ...
        output$check_analogs <- renderUI({
          p("Clustering cannot be performed.")
        })
        return()
      }
    ...
    if(!(grepl(".sdf", adds$name))){
      ...
      fpsim_cluster(fpset_added, lsm_rows, input$cluster_all_cmpds)
      ...
      output$distPlot <<- renderPlot({par(mar=c(5.1,0.5,4.1,0), xpd=FALSE)
...
########[-which(duplicated(df$V1)),]        
      ...
      # Clustering - No added compounds
        ...
        #  return()
        #}
        fpsim_cluster_no_added(lsm_rows,input$cluster_all_cmpds)
        #print("Got this far.")
        #incProgress(1/2, detail = "Clustering")
        #test_cluster_fpsim <- cbind(added_labels, test_bypass_fpsim)
        #colnames(test_bypass_fpsim) <- c("Compound", "LSM_ID", "Similarity")
...
########[-which(duplicated(df$V1)),]              
 ...
 # output$LINCSDownload_upload <- downloadHandler(
#    filename = function() {
#      'lincs_candidates.csv'
#    },
#    content = function(filename){
#      write.csv(display, filename, row.names = FALSE)
#    }
#  )
